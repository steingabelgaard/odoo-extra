...
class RunbotBuild(models.Model):
    ...
    def job_15_pylint(self, cr, uid, build, lock_path, log_path, args=None):
        ...
        if not os.path.isfile('%s_flake8.cfg' % (path_pylint_conf)):
        ...
        with open(fname_pylint_run_sh, "w") as f_pylint_run_sh:
            ...
            for module_to_check_pylint in modules_to_check_pylint:
                if not os.path.isfile(os.path.join(os.path.split(build.server("addons"))[0], module_to_check_pylint, '.nolint'):

                    cmd = "/usr/local/bin/flake8 --config=%s_flake8__init__.cfg %s" % \
                          (path_pylint_conf,
                           os.path.join(build.server('addons'),
                                    module_to_check_pylint))
                    f_pylint_run_sh.write(cmd + '\n')

                    cmd = "/usr/local/bin/flake8 --config=%s_flake8.cfg %s" % \
                          (path_pylint_conf,
                           os.path.join(build.server('addons'),
                                    module_to_check_pylint))
                    f_pylint_run_sh.write(cmd + '\n')

                    cmd = "/usr/local/bin/pylint --load-plugins=pylint_odoo --rcfile=%s_pylint_pr.cfg %s" % \
                          (path_pylint_conf,
                           os.path.join(build.server('addons'),
                                    module_to_check_pylint))
                    f_pylint_run_sh.write(cmd + '\n')
